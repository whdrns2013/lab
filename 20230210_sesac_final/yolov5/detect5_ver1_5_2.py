...
5-3. 실행
- cmd 등의 명령 프롬프트를 통해 본 파일이 있는 곳으로 디렉토리 이동한 뒤, 아래 명령어를 실행해주세요.
- python detect5_ver3_1_2.py --weights ./runs/best/best_2.pt --source 0 --save-txt --saving-img --detecting-time 3 --conf-thres 0.2
...
ver3.1.2 : 이미지 저장 규칙 수정 (모든 이미지 저장 -> 첫 탐지 이미지와 퇴장 x초 후 마지막 이미지만 저장)
...
%% To Do
- ver3.1.3 : 보안 이슈로 인한 불편사항 최소화 : 보안 통과하지 않더라도 최소한의 기능은 이용 가능하게, 보안사항 입력 간편하게.
- ver3.1.4 : log 및 이미지에 user-uid, cam-name 부여를 통해 실제 서비스처럼 꾸미기
- ver3.1.5 : 로그값 데이터프레임화 -> RDS 전송을 묶어 다른 스레드에서 처리하게끔 효율화
