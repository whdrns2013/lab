...
5-3. 실행
- cmd 등의 명령 프롬프트를 통해 본 파일이 있는 곳으로 디렉토리 이동한 뒤, 아래 명령어를 실행해주세요.
- python detect5_ver3_0_1.py --weights ./runs/best/best_2.pt --source 0 --save-txt --saving-img --detecting-time 3 --conf-thres 0.2
...
6-1. 명령어 옵션
- python detect5_ver3_1.py를 실행할 때의 옵션을 소개한다.
...
- RDS 및 S3 연결 보안 강화 : 환경변수를 이용한 비노출
- RDS 연결시 노출될 수 있는 정보 : RDS 엔드포인트, RDS 포트번호, RDS 데이터베이스 이름, RDS 관리자 계정 명, RDS 관리자 계정 암호
- 환경변수는 ignore를 통해 비노출 되는 환경에서 공유 필

%% 성능개선

%% To Do
- ver3.0.2 : 보안 이슈로 인한 불편사항 최소화 : 보안 통과하지 않더라도 최소한의 기능은 이용 가능하게, 보안사항 입력 간편하게.
- ver3.1.0 : log 및 이미지에 user-uid, cam-name 부여를 통해 실제 서비스처럼 꾸미기
- ver3.1.1 : 로그값 데이터프레임화 -> RDS 전송을 묶어 다른 스레드에서 처리하게끔 효율화
- ver3.1.2 : 퇴치부 알람 테스트 구현
- ver4.0.0 : 퇴치부 알람 구현
