...
5-3. preference 세팅
- 아래 코드 도입부의 preference를 세팅해주시기 바랍니다.

5-3. 실행
- cmd 등의 명령 프롬프트를 통해 본 파일이 있는 곳으로 디렉토리 이동한 뒤, 아래 명령어를 실행해주세요.
- python detect5_ver3_0_0.py --weights ./runs/best/best_2.pt --source 0 --save-txt --saving-img --detecting-time 3 --conf-thres 0.2
...
- RDS로 로그 데이터 전송 기능 구현
- S3로 이미지 전송 기능 구현
- 성능 개선 : 객체 인식 외 부가 기능(DB로 데이터 전송) 멀티스레드로 구현

%% 성능개선
1. 멀티 스레드 병렬처리를 통한 YOLOv5 끊김(대기) 현상 해결
- 기존에는 RDS로의 업로드 시간동안 YOLOv5의 탐지 모델이 일시 중지(대기)되는 문제 발생
- 이는 기존 YOLO가 구동되는 thred에 RDS 전송 업무를 올렸기 때문에 당연한 결과였음. 하나의 작업이 완료되기 전까지는 다음 작업이 실행되지 않음.
- 해결을 위해 하나의 코드에서 여러 흐름을 실행할 수 있게 multi-thread 를 구현하여 YOLO detecting이 중단되는 일이 없도록 함
- 원래의 thread에는 YOLO의 detecting 흐름만이 있고, 객체 탐지 및 퇴장으로 이벤트가 종료되면 새로운 thread를 생성하여 해당 thread가 RDS 전송 작업을 맡게끔 함.
- 새로 생성된 thread는 맡았던 작업(RDS 전송)을 완료하면 자동으로 종료되므로 자원 낭비 없음.
- 이는 S3 업로드에도 동일하게 적용될 예정.

%% To Do
- ver3.0.1 : RDS 및 S3 연결 코드의 보안 강화
- ver3.0.2 : 로그값 데이터프레임화 -> RDS 전송을 묶어 다른 스레드에서 처리하게끔 효율화
- ver3.0.3 : log 및 이미지에 user-uid, cam-name 부여를 통해 실제 서비스처럼 꾸미기
- ver4.0.0 : 퇴치부 알람 구현
