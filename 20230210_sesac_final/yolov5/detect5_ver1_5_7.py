...
## 실행코드 : python detect5_ver1_5_7.py --weights ./runs/best/best_5.pt --source 0 --save-txt --saving-img --detecting-time 3 --conf-thres 0.2
...
5-3. 실행
- cmd 등의 명령 프롬프트를 통해 본 파일이 있는 곳으로 디렉토리 이동한 뒤, 아래 명령어를 실행해주세요.
- python detect5_ver1_5_4.py --weights ./runs/best/best_2.pt --source 0 --save-txt --saving-img --detecting-time 3 --conf-thres 0.1
...
ver1.5.4 : userId 추가, 데이터프레임 및 RDS에 userId 추가되도록
ver1.5.5 : streaming 메서드화
ver1.5.6 : save log,  save img 메서드 정비


############################################################################
############################### 업데이트노트 ##################################

%% detect5_ver1_5_7.py 주요 사항


%% 성능개선

%% To Do
- ver : 보안 이슈로 인한 불편사항 최소화 : 보안 통과하지 않더라도 최소한의 기능은 이용 가능하게, 보안사항 입력 간편하게.
- ver : log 및 이미지에 user-uid, cam-name 부여를 통해 실제 서비스처럼 꾸미기
- ver : 로그값 데이터프레임화 -> RDS 전송을 묶어 다른 스레드에서 처리하게끔 효율화
- ver : 퇴치부 알람 객체에 따라 다른 소리가 재생되도록 구현
...
# 보관용 : 이미지 처리 전체 흐름 - 추후 필요 없을 시 삭제
...
def save_log_method(txt_path, timestamp, s):
    with open(f'{txt_path}/{timestamp}.txt', 'a') as f:
        f.write(timestamp + s + '\n')


# 웹캠 등으로 streaming 되는 자료에 대한 처리 메서드
def stream_results(annotator, view_img, p, windows, bottom_center_list):        
    ...
    if view_img:
        ...
        for i, bc in enumerate(bottom_center_list):
            cv2.circle(im0, (bc[1], bc[1]), 3, (0, 0, 255), 4)  
...
def make_log_dataframe(path, event_name):
    ...
    censored_col_name = ['event_name', 'userId', 'detect_date', 'detect_time',
...
def run(
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # 종혁 추가 : bottom_center_list : 이벤트 종료시 비워져야 함
        bottom_center_list = []
        ...
        for i, det in enumerate(pred):  # per image
            ...
            for *xyxy, conf, cls in reversed(det):    
                ...
                # xyxy : 좌측 상단 xy, 우측 하단 xy
                print("xyxy : ", xyxy)
                bottom_center = [xyxy[3], int((xyxy[0] + xyxy[2])/2)]
                bottom_center_list.append(bottom_center)
                ...
                stream_results(annotator, view_img, p, windows, bottom_center_list)
            ...
            if (save_txt)|(saving_img): # 로그나 텍스트를 저장하기로 했다면
                ...
                try:
                    if (len(det) == 0)&(event_type == 0): # detect 감지 대상이 없는 상시 상태
                        ...
                        save_log_method(txt_path, timestamp, s)
                        if saving_img:
