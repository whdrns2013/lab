...
- 이미지는 이벤트 시간동안의 매 프레임마다의 이미지가 저장됨
...
5-2. AWS CLI 세팅 : AWS 자동저장 기능 사용시에 한함 (detect.py 버전 3 이상)
...
5-3. preference 세팅 : AWS 자동저장 기능 사용시에 한함 (detect.py 버전 3 이상)
- 아래 코드의 import 부 아래 preference를 세팅해주시기 바랍니다.
- preference 에는 AWS와 연결에 필요한 여러 정보들을 설정하는 부분입니다.

5-3. 실행
- cmd 등의 명령 프롬프트를 통해 본 파일이 있는 곳으로 디렉토리 이동한 뒤, 아래 명령어를 실행해주세요.
- python detect5_ver3_1_1.py --weights ./runs/best/best_2.pt --source 0 --save-txt --saving-img --detecting-time 3 --conf-thres 0.2
...
6-1. 명령어 옵션
- python detect5_ver3_1_1.py를 실행할 때의 옵션을 소개한다.
...
ver2.0.0 : 객체 탐지시 이미지 및 로그 저장 정식 구현
ver2.0.1 : 로그 및 이미지 저장 프로세스 최적화
ver2.0.2 : 로그 및 이미지 저장 코드 리펙토링
ver2.0.3 : parse opt 추가 - 저장 여부 선택 가능 ** AWS 연결 불가할 경우 이 버전 구동 추천
ver2.1.0 : gif 만들기 메서드, log의 dataframe화 구현, AWS 연결 테스트
ver3.0.0 : AWS RDS, S3와 파일 전송 구현, 멀티스레딩 구현
ver3.0.1 : AWS 연결 보안 강화 - env 이용
ver3.1.0 : 객체 탐지시 경보 울림
ver3.1.1 : 로그 규칙 재정의 (공백 포함하지 않도록)


############################################################################
############################### 업데이트노트 ##################################

%% detect5_ver3_1_1.py 주요 사항
- 로그 규칙 재정의
- 기존 event_name에서 공백을 포함하던 로그에서 -> 공백을 포함하지 않도록 조치
- 프론트 백엔드 부에서 RequestParam 값을 event_name으로 취급하는데, URL에는 공백이 들어가면 오류 발생
- 이에 대해 event_name에서 공백을 제외

%% 성능개선

%% To Do
- ver3.1.2 : 보안 이슈로 인한 불편사항 최소화 : 보안 통과하지 않더라도 최소한의 기능은 이용 가능하게, 보안사항 입력 간편하게.
- ver3.1.3 : log 및 이미지에 user-uid, cam-name 부여를 통해 실제 서비스처럼 꾸미기
- ver3.1.4 : 로그값 데이터프레임화 -> RDS 전송을 묶어 다른 스레드에서 처리하게끔 효율화
