                ...
                for detection in detections:
                    ...
                    if 'id' in detection:
                        ...
                        if current_obj_id != detection_id:
                            # if a new object is detected, create a new output video file and log file
                            if output is not None:
                                output.release()
                                log_file.close()
                            output_path = f"C:/Users/freef/Desktop/theos/easy-yolov7-main/output{detection_id}.mp4"
                            log_path = f"C:/Users/freef/Desktop/theos/easy-yolov7-main/log/{detection_id}_log.txt"
                            width = int(webcam.get(cv2.CAP_PROP_FRAME_WIDTH))
                            height = int(webcam.get(cv2.CAP_PROP_FRAME_HEIGHT))
                            fps = int(webcam.get(cv2.CAP_PROP_FPS))
                            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                        ...
                        # write log file
                        log_file.write(f"Object {detection_id}: x={detection['x']}, y={detection['y']}, w={detection['width']}, h={detection['height']}, t={webcam.get(cv2.CAP_PROP_POS_MSEC)}\n")
