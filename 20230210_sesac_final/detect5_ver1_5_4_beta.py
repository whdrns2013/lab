...
5-3. 실행
- cmd 등의 명령 프롬프트를 통해 본 파일이 있는 곳으로 디렉토리 이동한 뒤, 아래 명령어를 실행해주세요.
- python detect5_ver1_5_4_beta.py --weights ./runs/best/best_2.pt --source 0 --save-txt --saving-img --detecting-time 3 --conf-thres 0.2
...
def run(
        ...
        saving_img = True, # 종혁 : 각 프레임 이미지 저장 여부
        detecting_time = 10,
    ):
    source = str(source)
