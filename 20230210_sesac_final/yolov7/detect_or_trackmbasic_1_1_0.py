...
% ver1.1.0 주요 사항
- danger_range 내 탐지된 경우 알람 재생 기능
- streaming 되는 이미지에 4분할 선 및 y좌표 10등분 선 표시
- streaming 되는 이미지에 danger range 영역 표시

% To Do
- ver.1.1.1 : danger range를 코드 실행 opt 에서 설정할 수 있도록 업그레이드
...
def detect(save_img=True):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for x1,y1,x2,y2,conf,detclass in det.cpu().detach().numpy():
                    ...
                    if object_bot_center[1] >= danger_range:
                        run_sounding('/Users/jongya/Desktop/Workspace/lab/20230210_sesac_final/yolo7-codingbug/yolov7/beep.wav')
            ...
            # 종혁 테스트 : 이미지에 danger_range 표시되도록 하기
            if view_img:
                danger_range_ratio = 0.5
