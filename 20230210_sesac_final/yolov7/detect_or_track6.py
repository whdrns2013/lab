...
# def draw_minimap(im0, bbox_xyxy, identities, categories, names, colors, minimap_size=(150, 150)):
#     h, w, _ = im0.shape
#     minimap = np.zeros((minimap_size[1], minimap_size[0], 3), dtype=np.uint8)
    
#     scale_w = minimap_size[0] / w
#     scale_h = minimap_size[1] / h

#     for i, box in enumerate(bbox_xyxy):
#         x1, y1, x2, y2 = box
#         mini_x1, mini_y1 = int(x1 * scale_w), int(y1 * scale_h)
#         mini_x2, mini_y2 = int(x2 * scale_w), int(y2 * scale_h)

#         category = int(categories[i])
#         color = colors[category]
#         label = f'{names[category]}'
#         t_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, 1, 1)[0]
#         cv2.rectangle(minimap, (mini_x1, mini_y1), (mini_x2, mini_y2), color, 2)
#         cv2.rectangle(minimap, (mini_x1, mini_y1 - t_size[1] - 3), (mini_x1 + t_size[0], mini_y1), color, -1)
#         cv2.putText(minimap, label, (mini_x1, mini_y1 - 2), cv2.FONT_HERSHEY_PLAIN, 1, [255, 255, 255], 1)

#     # 웹캠 화면에 미니맵 추가
#     im0[5:5+minimap_size[1], w-minimap_size[0]-5:w-5] = minimap
