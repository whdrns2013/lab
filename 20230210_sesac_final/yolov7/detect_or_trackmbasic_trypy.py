...
def saving_logs(txt_path, event_name, s):
...
def tracking_object(det, names, colors, opt, im0):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            try:
                ...
                if len(det) == 0 : 
                    ...
                    im0 = tracking_object(det, names, colors, opt, im0)
                    cv2.imshow('img', im0)
                    cv2.waitKey(1)
                    pass
                
                # elif 객체가 없다가 처음 출현한 상태
                # - event_type = 1
                # - 이미지 저장
                # - 로그 저장
                # - 경보 재생
                if len(det)&(event_type == 0):
                    event_type = 1
                    print({f'event_type : {event_type}'})
                    im0 = tracking_object(det, names, colors, opt, im0)
                    saving_images(img_path, timestamp, im0)
                    saving_logs(txt_path, event_name, s)
                    # alarm -> 멀티 스레드
                    ...
                    # out = cv2.VideoWriter(img_path + f"{event_name}.mp4", fourcc, int(fps), (w, h), True)
                    
                    # ret, frame = capture.read()

                
                # elif 객체가 이어서 계속 탐지될 때
                # - event_type = 1
                # - 로그 저장
                if len(det)&(event_type == 1):
                    print({f'event_type : {event_type}'})
                    im0 = tracking_object(det, names, colors, opt, im0)
                    saving_logs(txt_path, event_name, s)
                    timestamp_old = timestamp
                ...
                # elif 객체가 있다가 없어졌을 때 + 없어진 지 10초 미만
                # - event_type = 2
                if (len(det) == 0)&(event_type == 1)&((timestamp - timestamp_old) < detecting_time):
                    event_type = 2
                    print({f'event_type : {event_type}'})
                    im0 = tracking_object(det, names, colors, opt, im0)
                    saving_logs(txt_path, event_name, s)
                ...
                # - RDS 전송
                # - S3 전송
                if (len(det) == 0)&(event_type == 2)&((timestamp - timestamp_old) > detecting_time):
                    event_type = 0
                    print({f'event_type : {event_type}'})
                    im0 = tracking_object(det, names, colors, opt, im0)
                    saving_logs(txt_path, event_name, s)
