...
0. 실행 코드
0-1. 주간
python detect_or_trackmbasic_1_3_7.py --weights coco2wild4_day_31epoch_best.pt --view-img --track --show-track --unique-track-color --danger-range 0.5 --conf-thres 0.7 --detecting-time 10 --thickness 3 --source 0 --class 1 2 3
0-2. 야간
python detect_or_trackmbasic_1_3_7.py --weights coco2wild4_night_12epoch_best.pt --view-img --track --show-track --unique-track-color --danger-range 0.5 --conf-thres 0.7 --detecting-time 10 --thickness 3 --source 0 --class 0 1 2
python detect_or_trackmbasic_1_3_7.py --weights coco2wild4_day_31epoch_best.pt --no-trace --track --show-track --unique-track-color --danger-range 0.5 --conf-thres 0.7 --detecting-time 10 --thickness 3 --source 0 --class 1 2 3
...
% 명령어
- python detect.py를 실행할 때의 옵션을 소개한다.
...
ver1.3.3 : 속도 개선 테스트
ver1.3.5 : sound 처리 모듈 변경 (playsound -> pydub), 데이터프레임 오류 해결
ver1.3.6 : 알람 메서드 정비
ver1.3.7 : 동영상 저장 코덱 변경 (mp4v -> h264) : 크롬에서 동영상 재생 되지 않는 문제 해결
...
# 종혁 추가 : global 변수
alarm_path = '/Users/jongya/Desktop/Workspace/lab/20230210_sesac_final/yolo7-codingbug/yolov7/sound2.wav'
userId = "yoon" # 사용자 id
...
def make_log_dataframe(path, event_name):
    ...
    origin_col_name = ['event_name', 'userId', 'detect_date', 'detect_time',
...
def make_danger_range_display(danger_range_ratio, im0):
    ...
    for i in range(11):
        ...
        cv2.line(im0, (0, recog*i), (int(im0.shape[1]/20), recog*i), (153, 255, 255), 3)
        cv2.line(im0, (int(im0.shape[1]), recog*i), (int(im0.shape[1] - im0.shape[1]/20), recog*i), (153, 255, 255), 3)
    ...
    if opt.track:
        ...
        if opt.show_track:
            ...
            for t, track in enumerate(tracks):
                ...
                track_color, thickness=opt.thickness, tipLength=0.3) 
...
def saving_images(img_path, im0, event_type):
    ...
    cv2.imwrite(img_path + '/' + img_name + '.jpg', im0)

def saving_videos(vid_writer, vid_path, save_dir, im0, vid_cap=None, event_name='1'):
    if vid_path is None or vid_writer is None:
        ...
        vid_writer = cv2.VideoWriter(str(save_dir / f'detect_vid.mp4'), cv2.VideoWriter_fourcc(*'mp4v'), fps, (w, h))
            ...
            if len(det) == 0:
                ...
                if event_type == 0:
                    ...
                    if (time_stamp - time_stamp_old) <= detecting_time:  # 객체 사라짐, dt 이하
                        ...
                        im0 = track_track(det, colors, names, im0, img)
                        saving_images(img_path, im0, event_type)
                        ...
                        run_to_s3(s3_resource, s3_bucket_name, event_name, vid_path)
                ...
                if event_type >= 1: # 지속 탐지
                    ...
                    # 경보 알람
                    if(time_stamp - alarm_time) > 5:
                        for x1,y1,x2,y2,conf,detclass in det.cpu().detach().numpy():
                            object_bot_center = [int((x1 + x2)/2), int(y2)]
                            danger_range_ratio = opt.danger_range_ratio
                            danger_range = int(im0.shape[0] * (1- danger_range_ratio))
                            print('object_center : ', object_bot_center)
                            print('danger_range : ', danger_range)
                            if (object_bot_center[1] >= danger_range):
                                run_sounding(alarm_path)
                                alarm_time = now.timestamp()
                    ...
                    if not os.path.exists(str(save_dir / event_name)):
                        ...
                        vid_path = str(save_dir / event_name) + '/' + 'vid'
                        os.makedirs(img_path)
                    ...
                    # 비디오 작성 시작
                    vid_writer, vid_path = saving_videos(vid_writer, vid_path, save_dir / event_name / 'vid', im0, vid_cap, event_name=event_name)
