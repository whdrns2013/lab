...
python detect_or_trackmbasic_1_3_4.py --weights wild5_retry.pt --no-trace --view-img --source 0 --track --show-track --unique-track-color --danger-range 0.5 --conf-thres 0.7 --detecting-time 10
...
def saving_videos(vid_writer, vid_path, save_dir, im0, vid_cap=None):
    if vid_path is None or vid_writer is None:
        ...
        vid_path = str(save_dir / f'{time.time()}.mp4')
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det) == 0:
                ...
                if event_type >= 1: # 지속 탐지
                    ...
                    # 비디오 작성 시작
                    vid_writer, vid_path = saving_videos(vid_writer, vid_path, save_dir / event_name / 'vid', im0, vid_cap)
