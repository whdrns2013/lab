...
python detect_or_trackmbasic_1_2_1.py --weights best.pt --no-trace --view-img --source 0 --track --show-track --unique-track-color --danger-range 0.5 --conf-thres 0.7
...
def detect(save_img=True):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # for x1,y1,x2,y2,conf,detclass in det.cpu().detach().numpy():
            #     bc = [int((x1 + x2)//2), int(y2)]
            #     temp_list.append(bc)
                
            # for i, bc in enumerate(temp_list):
            #     cv2.circle(im0, (bc[1], bc[0]), 3, (0, 0, 255), -1)
            #     if i >= 1:
            #         cv2.line(im0, (temp_list[i][1], temp_list[i][0]), (temp_list[i-1][1], temp_list[i-1][0]), (0, 0, 255), 2)
