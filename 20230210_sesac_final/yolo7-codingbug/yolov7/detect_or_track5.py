        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    time_diff = (now - startTime).total_seconds()
                    n = (det[:, -1] == c).sum()  # detections per class
                    s += f"{n} {names[int(c)]}{'s' * (n > 1)}, "  # add to string
                    
                    if time_diff >= opt.save-interval:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-interval', type=int, default=1, help='Save images and txt files every n seconds') #초 단위 저장추가
