...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            # 종료 조건 추가
            if len(det) == 0 and dataset.mode != 'image':  # 탐지된 객체가 없으면서, 이미지가 아닌 경우(비디오 또는 스트림)에만 종료 조건을 검사합니다.
                if webcam:  # 웹캠이 켜져 있으면 계속해서 객체를 탐지합니다.
                    continue
                else:  # 웹캠이 꺼져 있으면, 화면을 종료하기 위해 키보드 입력 대기합니다.
                    cv2.waitKey(0)
                    break
