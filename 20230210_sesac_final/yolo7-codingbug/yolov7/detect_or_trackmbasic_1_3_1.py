...
python detect_or_trackmbasic_1_3_1.py --weights best.pt --no-trace --view-img --source 0 --track --show-track --unique-track-color --danger-range 0.5 --conf-thres 0.7 --detecting-time 10
...
% ver1.3.1 주요 사항
- 이미지 저장, 로그 저장 알고리즘 재구현

% To Do
- 알람 울리게 개선
- log에 객체 수 나오도록 개선 필요
- trace 가 실제 객체 위치와 다른 점 개선 필요
- 비디오 기록되도록
- 이벤트 나누기 알고리즘 재현
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det) == 0:
                ...
                if event_type == 0:
                    ...
                    if (time_stamp - time_stamp_old) <= detecting_time:
                        event_type = 2
                        print({f'event_type : {event_type}'})
                        saving_logs(txt_path, s, now, det, names)
                    
                    elif (time_stamp - time_stamp_old) > detecting_time:
                        event_type = 0
                        print({f'event_type : {event_type}'})
